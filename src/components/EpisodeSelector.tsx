              ...
              <div className='flex-1 overflow-y-auto space-y-2 pb-20'>
                {(() => {
                  const sorted = [...availableSources].sort((a, b) => {
                    ...
                    if (!aIsCurrent && bIsCurrent) return 1;
                    const ga = a.source_group || '其他';
                    const gb = b.source_group || '其他';
                    return ga.localeCompare(gb, 'zh');
                  });
                  const groups: Record<string, typeof sorted> = {};
                  sorted.forEach((s) => {
                    const g = s.source_group || '其他';
                  ...
                  return Object.entries(groups).map(([groupName, items]) => (
                    <div key={groupName}>
                      <div className='sticky top-0 z-10 px-2 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm border-b border-gray-100 dark:border-gray-800 mb-1'>
                        {groupName}
                      </div>
                      {items.map((source) => {
